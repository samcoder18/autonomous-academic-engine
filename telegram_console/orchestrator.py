...
class WorkflowOrchestrator:
    ...
    def _sync_article_runtime_state(
        ...
        readiness_status = _merge_article_readiness_statuses(
            self._parse_article_readiness_status(output_text),
            *_artifact_readiness_statuses(artifact_texts),
        )
    ...
    def _classify_article_blockers(
        ...
        *,
        ...
        blockers = list(_extract_article_artifact_blockers(artifact_texts or {}))
...
def _merge_article_readiness_statuses(*statuses: str | None) -> str | None:
    resolved: str | None = None
    severity = -1
    for status in statuses:
        if status not in ARTICLE_STATUS_SEVERITY:
            continue
        current = ARTICLE_STATUS_SEVERITY[status]
        if current > severity:
            resolved = status
            severity = current
    return resolved
...
def _artifact_readiness_statuses(artifact_texts: dict[str, str]) -> tuple[str, ...]:
    statuses: list[str] = []
    for source_name in ("review", "checklist"):
        for field_key, field_value in _iter_article_artifact_fields(artifact_texts.get(source_name, "")):
...
def _extract_article_artifact_blockers(artifact_texts: dict[str, str]) -> tuple[Blocker, ...]:
...
def _infer_artifact_blocker_category(field_key: str, field_value: str) -> str:
    ...
    if any(token in text for token in ("inferential", "gap", "counterargument", "logic", "overclaim", "аргумент", "контрарг", "вывод")):
        return "logic"
...
def _iter_article_artifact_fields(text: str) -> list[tuple[str, str]]:
    ...
    for raw_line in text.splitlines():
        ...
        if not match:
            continue
        field_key = _normalize_artifact_field_key(match.group(1))
...
def _parse_article_status_field(value: str) -> str | None:
    normalized = _normalize_artifact_value(value).casefold()
