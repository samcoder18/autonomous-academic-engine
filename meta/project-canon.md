# Compatibility Note

Этот файл сохранен только для legacy-ссылок и старых навыков.

Активный канон thesis lane больше не хранится в `meta/`.
Используй:

- [works/biometrics-vkr/work-canon.md](/Users/albina/дипломная/works/biometrics-vkr/work-canon.md) для текущей работы по умолчанию;
- `works/<slug>/work-canon.md` для любой другой активной работы.

Корневая конфигурация workspace:

- [workspace.toml](/Users/albina/дипломная/workspace.toml)
