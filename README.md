...
- Runtime state/lock/stop файлы под `output/telegram/runtime/autonomous/` пишутся атомарно; corrupted JSON на status surfaces нормализуется в fallback payload вместо CLI crash.
- JSON-first daemon/launchd/autonomous surfaces удерживают machine-readable contract: `kind`, `status`, `readiness_claim`; blocked/error payloads добавляют `stop_reason`.
